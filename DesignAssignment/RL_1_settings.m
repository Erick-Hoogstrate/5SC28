...
basename = 'Design_Assignment_';          % filename used for saving data

%% 1. Define state and important indices

% 1a. State vector (including all augmentations)
%
%  1  dtheta     angular velocity of the pendulum
%  2  theta      angle of the pendulum
%  3  sin(theta) cartesian representation of theta [augmentation]
%  4  cos(theta) cartesian representation of theta [augmentation]
%  5  u          force applied to disk [action]
%
...
mu0 = [0 0 0 0]';                  % initial state mean: encodes that the cart is in the middle of the track (with zero velocity) with the pendulum hanging down (with zero angular velocity).
...
% This is a structure used by the toolbox to describe the plant

% plant.dynamics = @Sol_1_dynamics;                 % function handle to the function that implements the ODE for simulating the system.
...
policy.p.hyp = log([1 1 1 0.7 0.7 1 0.01])';              % initialize policy hyper-parameters
...
%% 5. Set up the cost structure
cost.fcn = @Sol_1_loss;                       % cost function handle: saturating cost function (an unnormalized Gaussian subtracted from 1) [see Sol_3_loss.m]
