...
for r = 1:size(xx,1)
  ...
  if exist('j','var') && ~isempty(M{j})
    multi_draw_pendulum(latent{j}(r,2), latent{j}(r,end), cost,  ...
      ['trial # ' num2str(j+J) ', T=' num2str(H*dt) ' sec'], ...
      ['policy # ' num2str(policy_i) ', target at ' num2str(cost.target(2)*180/pi) ' degrees'], ...
      ['total experience (after this trial): ' num2str(dt*size(x,1)) ...
      ' sec'], M{j}(:,r), Sigma{j}(:,:,r));
  else
     multi_draw_pendulum(latent{jj}(r,2), latent{jj}(r,end), cost,  ...
